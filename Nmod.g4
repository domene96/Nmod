...
id_access:
  ( ID {c.insertStackOperand($ID.text)} ( LBRACKET {c.dimVarBegin()} exp {c.generateDimVarQuad()} ( COMMA {c.nextDimension($ID.text)} exp {c.generateDimVarQuad()} )* RBRACKET {c.dimVarEnd($ID.text)} )? );
modules:
